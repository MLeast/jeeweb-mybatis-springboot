...
import com.funcell.shop.sys.service.modules.sys.mapper.StoreUserMapper;
import com.funcell.shop.sys.service.modules.sys.service.IGoldLogService;
import com.funcell.shop.sys.service.modules.sys.service.IStoreUserService;
import com.funcell.shop.sys.service.utils.BigDecimalUtils;
import com.funcell.shop.sys.service.utils.JwtTokenUtil;
import com.funcell.shop.sys.service.utils.Log;
import com.funcell.shop.sys.service.utils.PrincipalUtils;
...
public class StoreUserServiceImpl extends CommonServiceImpl<StoreUserMapper, StoreUserEntity> implements IStoreUserService {
     ...
     */
    @Override
    public int updateGold(BigDecimal gold) {
        StoreUserEntity storeUserEntity = this.getUserInfo();
        storeUserEntity.setGold(BigDecimalUtils.add(gold.doubleValue(), storeUserEntity.getGold().doubleValue()));
        storeUserEntity.setGoldToday(BigDecimalUtils.add(gold.doubleValue(), storeUserEntity.getGoldToday().doubleValue()));//用户今日收获的金币
        return insertOrUpdate(storeUserEntity) ? 1 : 0;
    }
     ...
     */
    @Override
    public List<GoldLogEntity> getGoldList() {

        return goldLogService.getGoldList();
    }
     ...
     */
    @Override
    public List<StoreUserEntity> getFriendList() {

        return baseMapper.getFriendList(PrincipalUtils.getUsername());
    }
