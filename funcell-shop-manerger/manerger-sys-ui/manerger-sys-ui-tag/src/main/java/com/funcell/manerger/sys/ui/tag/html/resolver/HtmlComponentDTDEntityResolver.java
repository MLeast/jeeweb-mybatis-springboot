package com.funcell.manerger.sys.ui.tag.html.resolver;
...
public class HtmlComponentDTDEntityResolver implements EntityResolver, Serializable {
	...
	private static final String HOP_DYNAMIC_STATEMENT = "http://www.funcell.cn/dtd/";
		...
		if (systemId != null) {
			LOGGER.debug("trying to resolve system-id [" + systemId + "]");
 			if (systemId.startsWith(HOP_DYNAMIC_STATEMENT)) {
				LOGGER.debug(
						"recognized hop html component namespace attempting to resolve on classpath under cn/funcell/core/tags/html/dtd/");
