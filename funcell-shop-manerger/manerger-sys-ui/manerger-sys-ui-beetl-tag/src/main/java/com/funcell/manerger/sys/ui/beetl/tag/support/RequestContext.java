...
package com.funcell.manerger.sys.ui.beetl.tag.support;
...
public class RequestContext {
	...
	public RequestContext(HttpServletRequest request, @Nullable HttpServletResponse response,
		...
		if (this.webApplicationContext.containsBean(RequestContextUtils.REQUEST_DATA_VALUE_PROCESSOR_BEAN_NAME)) {
			this.requestDataValueProcessor = this.webApplicationContext.getBean(
					RequestContextUtils.REQUEST_DATA_VALUE_PROCESSOR_BEAN_NAME, org.springframework.web.servlet.support.RequestDataValueProcessor.class);
		}
	}
	...
	protected Object getModelObject(String modelName) {
		Object value = null;
		if (this.model != null) {
			value = this.model.get(modelName);
		}
		if (StringUtils.isEmpty(value)){
			value = this.request.getAttribute(modelName);
		}
		return value;
	}
